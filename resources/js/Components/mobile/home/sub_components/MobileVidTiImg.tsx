...
export default function MobileVidToImg({ className }: { className: string }) {
  ...
  return (
    <div className={className}>
      {showVideo && (
        <div className={`absolute top-0 z-20 w-full h-full transition-opacity duration-1000 ease-in-out ${opacityEffect ? 'opacity-0' : 'opacity-100'}`}>
          <video ref={videoRef} className="w-full h-full object-cover" autoPlay muted preload="auto">
      ...
      {opacityEffect &&
        <div className={`absolute w-full h-full top-0 flex items-center justify-center transition-opacity duration-1000 ease-in-out ${showVideo ? 'opacity-0' : 'opacity-100'}`}>
          <img src={motoImg} alt="Imagem moto da empresa" className="w-[80%] h-auto object-cover" />
        </div>
      }
      <div>
